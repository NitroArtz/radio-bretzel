...
exports.create = function (roles, userModel) {
  ...
  if (_.isEmpty(sails.config.permissions.adminLastname)) {
    throw new Error('sails.config.permissions.adminFirstname is not set');
  }
  if (_.isEmpty(sails.config.permissions.adminLastname)) {
    throw new Error('sails.config.permissions.adminLastname is not set');
  }
  return sails.models.user.findOne({ username: sails.config.permissions.adminUsername })
    .then(function (user) {
      ...
      return sails.models.user.register({
        ...
        owner: 1,
        model: userModel.id,
        firstName: sails.config.permissions.adminFirstname,
        lastName: sails.config.permissions.adminLastname,
      });
  });
};
